...
let
  ...
  startedHook = ''
    ...
    echo "${hostCoresMask}" > /proc/irq/default_smp_affinity
    for irq in /proc/irq/[0-9]*/smp_affinity; do 
      if [ $(cat $irq) = "${allCoresMask}" ]; then
        echo "${hostCoresMask}" > $irq 2> /dev/null 
      fi;
    done;
    for irq in $(cat /proc/interrupts | grep vfio | cut -d ":" -f 1); do 
      echo "${vmCoresMask}" > /proc/irq/$irq/smp_affinity; 
    done
  ...
  stopHook = ''
    ...
    echo "${allCoresMask}" > /proc/irq/default_smp_affinity
    for irq in /proc/irq/[0-9]*/smp_affinity; do 
      if [ $(cat $irq) = "${hostCoresMask}" ] || [ $(cat $irq) = "${vmCoresMask}" ]; then
        echo "${allCoresMask}" > $irq 2> /dev/null 
      fi;
    done;
